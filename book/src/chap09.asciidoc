...
Para los ejercicios de este capítulo necesitamos una lista de palabras en español. Hay muchas listas de palabras disponibles en la Web, pero la más adecuada para nuestro propósito es una de las listas de palabras recopiladas y contribuidas al dominio público por Ismael Olea (see http://olea.org/proyectos/lemarios/). Es una lista de 87900 palabras; en la colección Olea, el nombre del archivo es _lemario-general-del-espanol.txt. Puedes descargar una copia, con el nombre _palabras.txt_, desde https://github.com/PiensaEnJulia/PiensaEnJulia.jl/blob/master/data/palabras.txt.
(((lemario)))

Este archivo es un texto sin formato, por lo que puedes abrirlo con un editor de texto, o también puedes leerlo de Julia. La función incorporada +open+ toma el nombre del archivo como parámetro y devuelve una secuencia de archivos que puede usar para leer el archivo.
...
You can also use a file as part of a +for+ loop. This program reads +palabras.txt+ and prints each word, one per line:
...
Write a program that reads +palabras.txt+ and prints only the words with more than 20 characters (not counting whitespace).

[[ex09-2]]
==== Exercise 9-2

In 1927 Enrique Jardiel Poncela published five short stories, each ommiting a vowel. They include "El Chofer Nuevo", without the letter +'a'+, and "Un marido sin vocación", without the letter +'e'+. Since +'e'+ is the most common letter in Spanish, that’s not easy to do.
