...
For the exercises in this chapter we need a list of Spanish words. There are lots of word lists available on the Web, but the one most suitable for our purpose is one of the word lists collected and contributed to the public domain by Ismael Olea (see http://olea.org/proyectos/lemarios/). It is a list of 87900 words; in the Olea collection, the filename is _lemario-general-del-espanol.txt_; you can download a copy, with the simpler name _palabras.txt_, from https://github.com/PiensaEnJulia/PiensaEnJulia.jl/blob/master/data/palabras.txt.
(((lemario)))
