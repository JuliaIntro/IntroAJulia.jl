...
En lenguaje matemático, un diccionario representa un _mapeo_ de las claves a los valores, es decir, cada clave se asigna a un valor. A modo de ejemplo, crearemos un diccionario que asigne palabras en español a palabras en inglés. En este diccionario, las claves y los valores son todas cadenas.
(((mapping)))

A modo de ejemplo, crearemos un diccionario que traduzca palabras inglesas al espanol. En este diccionario, los
´ındices son strings (cadenas).

La función +Dict+ crea un nuevo diccionario sin elementos. Como +Dict+ es el nombre de una función integrada, debe evitar usarla como un nombre de variable.
...
El diccionario está vacio. Para agregar elementos al diccionario, se puede usar corchetes:
(((square brackets)))
...
Esta línea crea un elemento que asigna la clave "one" al valor "uno". Si imprimimos el diccionario nuevamente, vemos un par clave-valor con una flecha + pase: [= & gt;] + entre la clave y el valor:
