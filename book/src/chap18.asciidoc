...
Utilizar subtipos puede dificultar la depuración porque al llamar a una función con un objeto como argumento, puede ser difícil determinar qué método se invocará.

Supongamos que estamos escribiendo una función que funciona con objetos +Mano+. Nos gustaría que funcionara con todo tipo de +Mano+s, como +ManoDePoker+, +ManoDeBridge+, etc. Si invoca un método como +sort!+, Puede obtener el definido para un tipo abstracto + Hand +, pero si existe un método +sort!+ que tenga como argumento cualquiera de los subtipos, obtendrás esa versión. Este comportamiento suele ser algo bueno, pero puede ser confuso.
