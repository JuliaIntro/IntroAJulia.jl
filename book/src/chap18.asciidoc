...
Supongamos que estamos escribiendo una función que funciona con objetos +Mano+. Nos gustaría que funcionara con todo tipo de +Mano+ s, como +ManoDePoker+ , +ManoDeBridge+ , etc. Si invocas un método como +sort!+, podrías obtener el método definido para un tipo abstracto +Mano+ , pero si existe un método +sort!+ que tenga como argumento cualquiera de los subtipos, obtendrás esa versión. Este comportamiento suele ser algo bueno, pero puede ser confuso.
