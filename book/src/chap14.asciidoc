...
Puedes usar esto para almacenar no-cadenas en una base de datos.

[TIP]
====
De hecho, el almacenamiento de no-cadenas en una base de datos es tan común que se ha encapsulado en un paquete llamado + JLD2 + (consulte https://github.com/JuliaIO/JLD2.jl).
====
...
Si ejecuta este programa, se lee solo e imprime el número de líneas en el archivo, que es 9. También puede incluirlo en REPL de esta manera:
