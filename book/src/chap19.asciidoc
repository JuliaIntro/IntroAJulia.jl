...
La expresión que va antes de +?+ es una expresión de condición. Si la condición es +true+, se evalúa la expresión que va antes de +:+.e lo contrario, se evalúa la expresión que va después de +:+.
...
promoción::
Conversión de valores de diferentes tipos a un solo a tipo común.
(((promoción)))
