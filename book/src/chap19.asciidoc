...
Una _tarea_ es una estructura de control que puede pasar el control de forma cooperativa sin regresar. En Julia, una tarea puede implementarse como una función que tiene como primer argumento un objeto +Channel+. Se usa un channel para pasar valores de la función a la persona que llama.
...
El constructor +Channel+ crea la tarea. La función +fib+ se suspende después de cada llamada a +put!+ y se reanuda después de +take!+. Por rendimiento, se almacenan varios valores de la secuencia en el objeto channel durante un ciclo de reanudación/suspensión.
