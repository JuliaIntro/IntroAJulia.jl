...
En el <<chap10>> usamos un objeto de matriz como contenedor unidimensional con un índice para acceder a sus elementos. Sin embargo, en Julia, las matrices son colecciones multidimensionales.
...
Implementamos un tipo paramétrico sin campos + Fibonacci +, un constructor externo y dos métodos + iterar +. Se llama al primero para inicializar el iterador y devuelve una tupla que consta del primer valor, 0, y un estado. El estado en este caso es una tupla que contiene el segundo y el tercer valor, 1 y 1.
