...
Debido a que las palabras están en orden alfabético, podemos acelerar búsqueda de bisección (también conocida como búsqueda binaria), que es similar a lo que haces cuando buscas una palabra en el diccionario. Usted comienza en el medio y verifica si la palabra que está buscando va antes que la palabra en el medio de la matriz. Si es así, busca la primera mitad de la matriz de la misma manera. De lo contrario, busca la segunda mitad.

De cualquier manera, cortas el espacio de búsqueda restante a la mitad. Si la matriz de palabras tiene 113,809 palabras, se necesitarán unos 17 pasos para encontrar la palabra o concluir que no está allí.

Escriba una función llamada + inbisect + que tome una matriz ordenada y un valor objetivo y devuelva + verdadero + si la palabra está en la matriz y + falso + si no lo está.
