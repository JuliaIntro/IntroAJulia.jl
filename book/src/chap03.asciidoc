...
Este segundo ejemplo encuentra la razón seno de la variable +radianes+. El nombre de esta variable es una pista de que +sin+ y las otras funciones trigonométricas (+cos+, +tan+, etc.) toman argumentos en radianes. Para convertir grados a radianes, se debe dividir por 180 y multiplicar por latexmath:[\(\pi\)]:
...
Algunas de las funciones que hemos visto requieren argumentos. Por ejemplo, la función +cos+ necesita un número como argumento. Algunas funciones toman más de un argumento; +parse+ toma dos: un número y una cadena.
(((parámetro))) (((argumento))) (((analizar))) (((sin)))

Dentro de la función, los argumentos se asignan a variables llamadas _parametros_. A continuación se muestra un ejemplo de definición de función que toma un argumento:
...
Cuando +concatenar_dos+ termina, la variable +concatenar+ es eliminada. Si intentamos imprimirla, obtendremos error:
