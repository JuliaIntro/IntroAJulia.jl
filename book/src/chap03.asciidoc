...
Para realizar un seguimiento de qué variables se usan en qué lugares es útil dibujar un _diagrama de pila_. Al igual que los diagramas de estado, los diagramas de pila muestran el valor de cada variable, pero también muestran la función a la que pertenece cada una.
(((diagrama de pila))) ((("diagrama", "pila", see = "diagrama de pila")))

Cada función se representa por un _marco_. Un marco es un recuadro con el nombre de una función a un costado, y los parámetros y variables de la función adentro. El diagrama de pila para el ejemplo anterior se muestra en <<fig03-1>>.
(((marco)))
...
Algunas de las funciones que hemos utilizado, como las funciones matemáticas, devuelven resultados. A este tipo de funciones las llamaremos funciones productivas. Otras funciones, como +imprimirdosveces+, realizan una acción pero no devuelven un valor. Se llaman _funciones nulas_.
(((función productiva))) (((función nula)))

Cuando se llama a una función productiva, casi siempre queremos hacer algo con el resultado; por ejemplo, puede asignarlo a una variable o usarlo como parte de una expresión:
...
Pero en un script, si llama a una función productiva por sí sola, ¡el valor de retorno se pierde para siempre!
(((modo script)))
...
[TIP]
====
Use concatenación de cadenas y repetición. Además, Julia tiene una función integrada llamada +length+ que devuelve la longitud de una cadena, por lo que el valor de +length("monty")+ es 5.
...
(((imprimirgato))) ((("función", "definido por el programador", "imprimirgato", see = "imprimirgato")))
