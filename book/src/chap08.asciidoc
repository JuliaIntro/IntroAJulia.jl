...
Un cifrado César es una forma simple de cifrado que implica desplazar cada letra un número fijo de lugares. Girar una letra significa desplazarla a través del alfabeto, envolviéndola hasta el principio si es necesario, por lo que +'A'+ girado por 3 es + 'D' + y + 'Z' + girado por 1 es + 'A' + .
(((César cypher)))

Para rotar una palabra, rote cada letra en la misma cantidad. Por ejemplo, +"alegría"+ rotada por 7 es + "alegre" + y + "melón" + rotada por -10 es + "en cubos" +. En la película _2001: A Space Odyssey, la computadora de la nave_ se llama HAL, que es IBM girada por -1.

Escriba una función llamada + rotateword + que tome una cadena y un número entero como parámetros, y devuelva una nueva cadena que contenga las letras de la cadena original rotadas por la cantidad dada.
