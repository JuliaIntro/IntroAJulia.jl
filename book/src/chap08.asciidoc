...
[source,@julia-setup]
----
function encontrar(palabra, letra)
...
En cierto sentido, encontrar es lo contrario del operador +[]+. En lugar de tomar un índice y extraer el carácter correspondiente, toma un carácter y encuentra el índice donde aparece el carácter. Si el carácter no se encuentra, la función devuelve -1.
