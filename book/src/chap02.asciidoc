...
palabra clave::
Una palabra reservada que se utiliza para definir la sintaxis y estructura de un programa; no puede usar palabras clave como +if+, +function+ y +while+ como nombres de variables.
(((palabra clave)))
...
. En notación matemática puedes multiplicar +x+ e +y+ de esta manera: +x y+. ¿Qué pasa si intentas eso en Julia? ¿Y 5x?
