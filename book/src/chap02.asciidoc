...
Los programadores generalmente eligen nombres significativos para sus variables; documentan para qué se usa la variable.

Los nombres de las variables pueden ser tan largos como desee. Pueden contener casi todos los caracteres Unicode (consulte <<caracteres>>), pero no pueden comenzar con un número. Es válido usar letras mayúsculas, pero usar solo minúsculas para nombres de variables es la convención.
