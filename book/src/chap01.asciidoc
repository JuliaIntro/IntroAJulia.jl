...
Julia _REPL_ (de "Read–Eval–Print Loop" que se traduce en "Leer-Evaluar-Imprimir Bucle") ((("REPL (Read-Eval-Print Loop)")))es un programa que lee y ejecuta código de Julia. Puede iniciar REPL abriendo un terminal en JuliaBox y escribiendo *+julia+* en la línea de comando. Cuando comienza, debería ver una salida como esta:
...
Redundancia::
Para compensar la ambigüedad y reducir los malentendidos, los lenguajes naturales emplean mucha redundancia. Como resultado, a menudo son verboso. Los lenguajes formales son menos redundantes y más concisos.
