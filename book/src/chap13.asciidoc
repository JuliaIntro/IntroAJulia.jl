...
Modifique el programa anterior para que lea una lista de palabras y luego imprima todas las palabras del libro que no están en la lista de palabras. ¿Cuántos de ellos son errores tipográficos? ¿Cuántos de ellos son palabras comunes que deberían estar en la lista de palabras, y cuántos de ellos son términos realmente macabros?
...
Escriba una función llamada +escogerdelhistograma+ que tome un histograma como se define en <<dictionary_collection_counters>> y devuelva un valor aleatorio del histograma, elegido con probabilidad en proporción a la frecuencia. Por ejemplo, para este histograma:
...
You should attempt the previous exercises before you go on. You will also need https://github.com/PiensaEnJulia/PiensaEnJulia.jl/blob/master/data/DonQuijote.txt.
...
----
hist = processfile(joinpath("..", "data", "DonQuijote.txt"));
----

[source,julia]
----
hist = processfile("DonQuijote.txt");
----

This program reads _DonQuijote.txt_, which contains the text of _Don Quijote_ by  Miguel de Cervantes.
(((Austen, Jane)))
...
Total number of words: 385925

julia> println("Number of different words: ", differentwords(hist))
Number of different words: 23607
----
...
I use a tab character (+pass:['\t']+) as a “separator”, rather than a space, so the second column is lined up. Here are the results from _Don Quijote_:
(((separator)))(((\t)))
...
Finding the words from the book that are not in the word list from +palabras.txt+ is a problem you might recognize as set subtraction; that is, we want to find all the words from one set (the words in the book) that are not in the other (the words in the list).
...
To find the words in the book that are not in +palabras.txt+, we can use +processfile+ to build a histogram for +palabras.txt+, and then +subtract+:

[source,julia]
----
words = processfile("palabras.txt")
...
Here are some of the results from _Don Quijote_:
