...
A veces no se sabe que un ciclo debe terminar hasta que se llega a la mitad del cuerpo. En ese caso, se puede usar la _sentencia break_ para salir del bucle.
...
En cada iteración, se le pide al usuario una entrada con el símbolo ">". Si el usuario escribe +listo+, la sentencia break sale del bucle. De lo contrario, el programa repite lo que escriba el usuario y vuelve a la parte superior del bucle. A continuación se muestra su ejecución:
