...
Este capítulo presenta un tipo integrado más: las tuplas, y luego muestra cómo las matrices, los diccionarios y las tuplas funcionan en conjunto. También se presenta una característica útil para las matrices de argumentos de longitud variable: los operadores de recopilación y dispersión.
...
+máximo+ y +mínimo+ son funciones integradas que encuentran los elementos más grandes y más pequeños de una secuencia, respectivamente. +minmax+ calcula ambos y devuelve una tupla de dos valores. Otra opción para realizar esto, es utilizar la función integrada +extrema+, la cual es más eficiente.
...
El parámetro de recopilación puede tener cualquier nombre, pero +args+ es la convención. A continuación se muestra cómo funciona la función:
...
Muchas de las funciones integradas usan tuplas de argumento de longitud variable. Por ejemplo, +max+ y +min+ pueden tomar cualquier número de argumentos:
