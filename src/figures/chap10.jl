function fig10_1(output::Symbol, font::String, scale::Float64)
  p = TikzPicture(L"""
  \node[anchor=east](ch) at(-2.75, 0) {\tt quesos};
  ...
  \draw[-latex] (ch2) -- (chv2);
  \draw[-latex] (ch3) -- (chv3);
  \node[anchor=east](nu) at(-2.75, -1.75) {\tt numeros};
  ...
  \draw[-latex] (nu2) -- (nuv3);
  \node[anchor=east](em) at(-2.75, -3) {\tt vacio};
...
function fig10_5(output::Symbol, font::String, scale::Float64)
  ...
  \node[anchor=west] (l) at(-0.75, 0) {\tt letras};
  \node[anchor=east] at(-1.2,-1){\tt borrarprimero!};
