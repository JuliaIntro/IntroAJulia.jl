function fig12_1(output::Symbol, font::String, scale::Float64)
  ...
  \node[anchor=west] (av) at (-0.25, 0.25) {\tt "Cortázar"};
  \node[anchor=east] (b) at(-1.25, -0.25) {\tt 2};
  \node[anchor=west] (bv) at (-0.25, -0.25) {\tt "Julio"};
  \draw[-latex] (a) -- (av);
  \draw[-latex] (b) -- (bv);
...
function fig12_2(output::Symbol, font::String, scale::Float64)
  ...
  \draw[-latex](nc)--(c);
  \node[anchor=east](ng) at(-0.25,0.75) {\tt ("Mistral","Gabriela")};
  \node[anchor=west](g) at(0.75,0.75) {\tt "08700 100 222"};
  \draw[-latex](ng)--(g);
  \node[anchor=east](ni) at(-0.25,0.25) {\tt ("Vallejo","César")};
  \node[anchor=west](i) at(0.75,0.25) {\tt "08700 100 222"};
  \draw[-latex](ni)--(i);
  \node[anchor=east](nt) at(-0.25,-0.25) {\tt ("Sáenz","Jaime")};
  \node[anchor=west](t) at(0.75,-0.25) {\tt "08700 100 222"};
  \draw[-latex](nt)--(t);
  \node[anchor=east](nj) at(-0.25,-0.75) {\tt ("Asunción","José")};
  \node[anchor=west](j) at(0.75,-0.75) {\tt "08700 100 222"};
  \draw[-latex](nj)--(j);
  \node[anchor=east](np) at(-0.25,-1.25) {\tt ("Eloy","Andrés")};
